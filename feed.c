...
{
  printf("[ERRO] Recebido do manager : %s\n", errMsg->errorMenssagem);
}
...
void commSubscribe(char *topico)
{
  ...
  if(fd == -1)
  {
    printf("[ERRO] Falha ao abrir FIFO do manager!\n");
    return;
  }

  if(write(fd , &pedido , sizeof(pedido)) == -1)
  {
    printf("[ERRO] Falha ao enviar pedido de inscricao!\n");
  }
...
void commUnsubscribe(char *topico)
{
  ...
  if(write(fd , &pedido , sizeof(pedido)) == -1)
  {
    printf("[ERRO] Falha ao enviar pedido de desinscrição\n");
  }
